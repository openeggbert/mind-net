        ...
        const std::chrono::system_clock::time_point& tp) const
{
    ...
    if (!dom_unspec && !dow_unspec) {
        ...
        return dom_any ? true : dom_match;
    } else if (!dow_unspec) {
        return dow_any ? true : dow_match;
    } else {
        // both UNSPECIFIED – theoretically shouldn't happen, but treat as "match all"
        return true;
    }
}
...
CronExpr::previous_before(const std::chrono::system_clock::time_point& from) const
    {
        auto from_sec = time_point_cast<std::chrono::seconds>(from);
        auto t = from_sec - std::chrono::seconds(1l);
        ...
        throw std::runtime_error("CronExpr::previous_before: no matching time in 5 years");
    }
