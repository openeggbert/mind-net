...
namespace mindnet::plugins::core::models
{
    ...
    void History::from_values(const entity_fields& values)
    {
        ...
        table_name = text();
        record_id = number();
        operation = static_cast<mindnet::core::Crudl>(number());
        data_json = text();
        reason = text();
    }
    ...
    string History::validate()
    {
        ...
        validator_chain_vector list{
            ...
            [this] { return test_true(operation == mindnet::core::Crudl::List ? true : record_id != 0, "Record ID must not be 0");},
            [this] { return test_ne(cast64(operation), 0, HistoryColumns::OPERATION); },
            [this] { return test_true(operation == mindnet::core::Crudl::List ? true : !data_json.empty(), "data_json must not be empty");},
        };
